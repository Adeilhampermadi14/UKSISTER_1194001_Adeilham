...
mahasiswa = ["andre", "ayu", "sarah", "fajar", "naldi"]
...
def spawn_process():
    ...
    new_process = Process(target=daftar_mahasiswa, args=("Andre", queue_pendaftaran))
    new_process.start()
    time.sleep(3)  
...
def test_threads():
    ...
    thread1 = Thread(target=daftar_mahasiswa, args=("naldi", queue_pendaftaran))
    thread2 = Thread(target=daftar_mahasiswa, args=("sarah", queue_pendaftaran))
